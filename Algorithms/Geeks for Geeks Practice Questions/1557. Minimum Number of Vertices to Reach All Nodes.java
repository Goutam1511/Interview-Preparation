...
from node from(i) to node to(i). Find the smallest set of vertices from which all nodes in the graph are reachable. It's guaranteed that a unique
...
Solution : A node that does not have any incoming edge can only be reached by itself. Any other node with incoming edges can be reached
from some other node. We only have to count the number of nodes with zero incoming edges.
*/

class Solution {
    public List<Integer> findSmallestSetOfVertices(int n, List<List<Integer>> edges) {
        int[][] edgesCnt = new int[n][2];

        for (List<Integer> edge : edges) {
            edgesCnt[edge.get(0)][0]++;
            edgesCnt[edge.get(1)][1]++;
        }
        List<Integer> res = new ArrayList<>();
        for (int i = 0; i < n; i++) {
            if (edgesCnt[i][1] == 0) res.add(i);
        }
